...
# PJM: Yes it is necessary.  Above is the output file; Line 38 is the input file.  I removed the args.v from line 29 tho.  we only want output going to output directory (args.o). 
...
with open(args.v) as vcf:
    for line_idx, line in enumerate(vcf): #Cycle over lines in the VCF file
        ...
        # PJM: No you don't need it...is redundant
        ...
        if len(cols) < 2:               ## This should be info just before header
            ...
            # PJM: You don't need this.  This is the header of vcf file and the output lookup table does not need a header.  Just scaffold and position of each site that needs to be flipped
            # vcf.write(line)
            # newVCF.write(line)
                    ...
                    if j == 2 or j == 3:
                        if gt[0] != ".":
                            ...
                            if sum([int(x) for x in gt]) == 2:
                                alt_ind += 1
                                # if gt == '1/1': # PJM: You don't need this.  gt has been split by "/" on line 82, so will never equal '1/1'.  Line 92 accomplishses what you are trying to do here.
                                #     print 'blah2'
